    ...
    return new Promise(function(resolve,reject){
      ...
      let response = axios({
        ...
        if(res.data.hasOwnProperty('status_code') && res.data.status_code >= 300 && res.data.status_code != 407){
          reject(new MidtransError(`Midtrans API is returning API error. HTTP status code: ${res.data.status_code}. API response: ${JSON.stringify(res.data)}`))
        }
        resolve(res.data);
      }).catch(function(err){
        ...
        if(res.status >= 300){
          reject(new MidtransError(`Midtrans API is returning API error. HTTP status code: ${res.status}. API response: ${JSON.stringify(res.data)}`))
        }
        reject(err);
      })
    });
