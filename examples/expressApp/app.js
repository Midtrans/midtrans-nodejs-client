...
app.get('/simple_core_api_checkout', function (req, res) {
  res.render('simple_core_api_checkout', {
    clientKey: core.apiConfig.clientKey,
  });
});
...
function printExampleWarningMessage() {
  message =
    '<code><h4>Please set your server key and client key from sandbox</h4>In file: ' +
    __filename;
  message += '<br><br>// Set Your server key';
  message +=
    '<br>// You can find it in Merchant Portal -> Settings -> Access keys';
...
app.get('/core_api_credit_card_frontend_sample', function (req, res) {
  res.render('core_api_credit_card_frontend_sample', {
    clientKey: core.apiConfig.clientKey,
  });
});
app.listen(3000, () =>
  console.log(`ExpressApp listening on localhost port 3000!`)
);
